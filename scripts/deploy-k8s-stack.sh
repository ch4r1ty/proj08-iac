...
OPEN_DEMO_PAGES="${OPEN_DEMO_PAGES:-1}"
if [[ "${OPEN_DEMO_PAGES}" == "1" || "${OPEN_DEMO_PAGES}" == "true" ]]; then
  "${SCRIPT_DIR}/open-demo-pages.sh" "${FLOATING_IP}" || true
else
  echo "Tip: run ${SCRIPT_DIR}/check-public-services.sh ${FLOATING_IP} --open to open the demo app, API docs, and Grafana dashboards."
fi
